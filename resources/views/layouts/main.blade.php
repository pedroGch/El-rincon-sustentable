...
<body>
  <div id="app" class="min-h-screen flex flex-col">
    ...
    <nav class="relative flex w-full flex-nowrap items-center justify-between  bg-principal py-2 text-white shadow-lg lg:flex-wrap lg:justify-start lg:py-4" data-te-navbar-ref>
      <div class="flex w-full flex-wrap items-center justify-between px-3">
        ...
        <!-- DESPLEGABLE -->
        <div class="!visible mt-2 hidden flex-grow basis-[100%] items-center lg:mt-0 lg:!flex lg:basis-auto" id="navbarSupportedContent14" data-te-collapse-item>
          <ul class="list-style-none mr-auto flex flex-col pl-0 lg:mt-1 lg:flex-row" data-te-navbar-nav-ref>
            ...
            <li class="my-4 pl-2 lg:my-0 lg:pl-2 lg:pr-1" data-te-nav-item-ref>
              <a class="active lg:px-2  text-white" aria-current="page" href="<?= url('/iniciar_sesion') ?>" data-te-nav-link-ref>Iniciar sesión</a>
            </li>
            @endif
          </ul>
          @if (auth()->check())
          ...
          <!-- BOTÓN USUARIO LOGUEADO -->
            ...
            @if (auth()->check())
              <div class="relative me-10" data-te-dropdown-ref>
                <button class="flex items-center whitespace-nowrap rounded px-6 pb-2 pt-2.5 text-xs font-medium uppercase leading-normal text-white shadow-[0_4px_9px_-4px_#3b71ca] transition duration-150 ease-in-out  hover:shadow-[0_8px_9px_-4px_rgba(59,113,202,0.3),0_4px_18px_0_rgba(59,113,202,0.2)]  focus:shadow-[0_8px_9px_-4px_rgba(59,113,202,0.3),0_4px_18px_0_rgba(59,113,202,0.2)] focus:outline-none focus:ring-0 active:shadow-[0_8px_9px_-4px_rgba(59,113,202,0.3),0_4px_18px_0_rgba(59,113,202,0.2)] motion-reduce:transition-none dark:shadow-[0_4px_9px_-4px_rgba(59,113,202,0.5)] dark:hover:shadow-[0_8px_9px_-4px_rgba(59,113,202,0.2),0_4px_18px_0_rgba(59,113,202,0.1)] dark:focus:shadow-[0_8px_9px_-4px_rgba(59,113,202,0.2),0_4px_18px_0_rgba(59,113,202,0.1)] dark:active:shadow-[0_8px_9px_-4px_rgba(59,113,202,0.2),0_4px_18px_0_rgba(59,113,202,0.1)]"
                type="button" id="dropdownMenuButton1d" data-te-dropdown-toggle-ref aria-expanded="false" data-te-ripple-init data-te-ripple-color="light"> <?= Auth::user()->name ?>
                  <span class="ml-2 w-2">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="h-5 w-5">
                        <path fill-rule="evenodd" d="M5.23 7.21a.75.75 0 011.06.02L10 11.168l3.71-3.938a.75.75 0 111.08 1.04l-4.25 4.5a.75.75 0 01-1.08 0l-4.25-4.5a.75.75 0 01.02-1.06z" clip-rule="evenodd" />
                    </svg>
                ...
                @if(Auth::user()->rol != 'admin')
                  <li>
                    <a class="block w-full whitespace-nowrap bg-transparent px-4 py-2 text-sm font-normal text-neutral-700 hover:bg-neutral-100 active:text-neutral-800 active:no-underline disabled:pointer-events-none disabled:bg-transparent disabled:text-neutral-400 dark:text-neutral-200 dark:hover:bg-neutral-600" href="<?= url('/perfil_usuario') ?>" data-te-dropdown-item-ref>Mi perfil</a>
                  </li>
                @endif
                  <li>
                    <hr class="my-2 h-0 border border-t-0 border-solid border-neutral-700 opacity-25 dark:border-neutral-200" />
    ...
    @if (\Session::has('status.message'))
      <div class="mt-5 mb-3 inline-flex w-full items-center rounded-lg bg-{{ \Session::get('status.type') ? \Session::get('status.type') : 'success' }}-100 px-6 py-5 bg- text-base text-{{ \Session::get('status.type') ? \Session::get('status.type') : 'success' }}-700" role="alert">
        <span class="mr-2">
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="h-5 w-5">
            <path fill-rule="evenodd" d="{{ \Session::get('status.svg', 'M2.25 12c0-5.385 4.365-9.75 9.75-9.75s9.75 4.365 9.75 9.75-4.365 9.75-9.75 9.75S2.25 17.385 2.25 12zm13.36-1.814a.75.75 0 10-1.22-.872l-3.236 4.53L9.53 12.22a.75.75 0 00-1.06 1.06l2.25 2.25a.75.75 0 001.14-.094l3.75-5.25z') }}" clip-rule="evenodd" />
          </svg>
        </span>
        <div>
    ...
    @endif
    <!-- ESPACIO DINÁMICO PARA TEMPLATES -->
    @yield('content')
    </main>
      <footer class="lg:py-6 lg:h-[360px] bg-secundario drop-shadow-xl text-white">
        <div>
          <div class="lg:flex mx-auto max-w-screen-xl">
            ...
            <div class="p-4 lg:p-0 lg:flex-1 ">
              ...
              <div class="hidden lg:flex align-middle justify-center lg:justify-normal lg:mb-4">
                <img src="{{ url('img/logo-davinci.png') }}" alt="logo Escuela Da Vinci" />
              </div>
            </div>
            <div id="container-team" class="lg:flex-1 p-4">
              ...
              <div class="flex flex-wrap pb-2 justify-center">
                <div id="avatar-pedro" class="text-center px-6 pt-8">
                  <img src="{{ url('img/avatar/avatar-pedro.jpg') }}" class="mx-auto mb-4 w-32 rounded-lg" alt="Avatar Pedro Gonzalez Chavez" />
                ...
                <div id="avatar-rocio" class="text-center px-6 pt-8">
                  <img src="{{ url('img/avatar/avatar-rocio.jpg') }}" class="mx-auto mb-4 w-32 rounded-lg" alt="Avatar Rocío Scotto" />
                ...
                <div id="avatar-valdi" class="text-center px-6 pt-8">
                  <img src="{{ url('img/avatar/avatar-valdi.jpg') }}" class="mx-auto mb-4 w-32 rounded-lg" alt="Avatar Ezequiel Valdiviezo" />
        ...
        <div class="sm:hidden">
          <div class="flex align-middle justify-center lg:justify-normal lg:mb-4">
            <img src="{{ url('img/logo-davinci.png') }}" alt="logo Escuela Da Vinci" />
          </div>
        </div>
      </footer>
