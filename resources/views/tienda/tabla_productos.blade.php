        ...
        <tr class="border-2">
          ...
          <td class="p-3 border-2">
            <form action="{{ url('/tienda/' . $producto->id . '/editar') }}" method="GET">
              <button type="submit" class="w-full botonPersonalizado-text-xs">Editar</button>
            </form>
            <button type="button" onclick="borrarProducto({{ $producto->id }}, '{{ $producto->nombre_prod }}')" class=" w-full botonPersonalizado-text-xs-rojo">Eliminar
            </button>
            <form action="{{ route('detalle.producto', ['id' => $producto->id]) }}" method="GET">
              <button type="submit" class="w-full text-xs botonPersonalizado-text-xs">Leer más</button>
            </form>
          </td>
        </tr>
      @endif
