                    ...
                    <table class="table">
                        ...
                        <tbody>
                            @forelse ($productos as $producto)
                                <tr>
                                    <td class="align-middle"><a href="{{ url('/producto/' . $producto->id) }}"><img
                                                src="{{ asset('./storage/' . $producto->imagen_prod) }}"
                                                alt="{{ $producto->alt }}" class="img-fluid shadow-sm" width="100"></a>
                                    </td>
                                    <td class="align-middle">{{ $producto->nombre_prod }}</td>
                                    <td class="align-middle">{{ $producto->precio_formateado($producto->precio) }}</td>
                                    <td class="align-middle" width="10%">
                                        <label for="cantidad_" class="visually-hidden">Cantidad</label>
                                        <input type="number" name="cantidad[]" id="cantidad_" value="1"
                                            class="form-control">
                                    </td>
                                    <td class="align-middle">{{ $producto->precio_formateado($producto->precio) }}</td>
