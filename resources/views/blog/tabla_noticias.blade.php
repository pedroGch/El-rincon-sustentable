      ...
      @foreach ($noticias as $noticia)
        <tr class="border-2">
          ...
          <td class="p-3 border-2">
            ...
            <button type="button" onclick="borrarNoticia({{ $noticia->id }})" class="w-full botonPersonalizado-text-xs-rojo">Eliminar</button>
            <form action="{{ url('/blog/' . $noticia->id . '/leer_mas') }}" method="GET">
              <button type="submit" class="w-full botonPersonalizado-text-xs">Leer más</button>
            </form>
          </td>
        </tr>
      @endforeach
