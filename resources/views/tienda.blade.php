  ...
  @foreach ($productos as $producto)
    ...
    <div class="flex flex-col w-full lg:w-1/4 md:w-1/2 p-4">
      <div class="block rounded-lg bg-white shadow-[0_2px_15px_-3px_rgba(0,0,0,0.07),0_10px_20px_-2px_rgba(0,0,0,0.04)] dark:bg-neutral-700">
        <div class="relative overflow-hidden bg-cover bg-no-repeat mx-auto" data-te-ripple-init data-te-ripple-color="light">
          <hr>
        ...
        <div class="p-6">
          ...
          <p class="mb-4 text-base text-neutral-600 dark:text-neutral-200">{{ $producto->descripcion_reducida() }}</p>
          <p class="w-full border-b-2 border-neutral-100 border-opacity-100 px-4 py-3 dark:border-opacity-50"><strong>{{ $producto->precio_formateado($producto->precio) }}</strong></p>
          <form action="{{ route('detalle.producto', ['id' => $producto->id]) }}" method="GET">
            <button type="submit" class="w-full botonPersonalizado" data-te-ripple-init data-te-ripple-color="light">Ver más
            </button>
          </form>
        </div>
      </div>
